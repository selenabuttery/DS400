...
ggplot(running, aes(x = age)) +
  ...
  labs(title = "Age and Net Time", x = "age", y = "net") + 
  theme_minimal()  
```

```{r}
ggplot(running, aes(x = year)) +
  geom_bar(fill = "orange", color = "black") + 
  labs(title = "Year and Age", x = "year", y = "age") + 
  theme_minimal()  
```
...
ggplot(running, aes(x = age, y = net)) +
  ...
  labs(title = "Scatterplot of Age of Runner and the time it took to finish the race", x = "Age of Runner", y = "Time in minutes of completion") +  
  theme_minimal()  
```

```{r}
ggplot(data = running, aes(y = net, x = age)) + 
  geom_point() +
  geom_smooth() +
  stat_cor()
```
