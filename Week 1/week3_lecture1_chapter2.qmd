...
likelihood_article_fake_has_excl) /
prior_probability_has_excl
```
...
```{r}
ggplot(data = pop_vs_soda, aes(x = state, fill = region)) + 
  geom_bar()
```
...
```{r}
prior_probability_midwest <- 0.645
```

```{r}
prior_probability_northeast <- .273
```
